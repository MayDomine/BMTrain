...
def make_distributed(model : torch.nn.Module):
    ...
    for kw in list(model._modules.keys()):
        if isinstance(model, torch.nn.ModuleList):
            if not isinstance(model._modules[kw], ZeROBlock):
                model._modules[kw] = ZeROBlock(model_wrapper_dispatch(model._modules[kw]))
        else:
...
def BMTrainModelWrapper(model : torch.nn.Module) -> torch.nn.Module:
    ...
    Replaces all parameters with DistributedParameter, all modules with DistributedModule, and modules in ModuleList with ZeROBlock.
    """
